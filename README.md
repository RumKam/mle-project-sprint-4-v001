...
Код для получения рекомендаций первой части проекта находится в файле `mle-project-sprint-4-v001/recommendations_v1.ipynb`.

# Сервис рекомендаций

Поскольку файлы с рекомендациями достаточно объемные, необходимые для тестирования сервиса на нескольких пользователях рекомендации находятся в файлах `test_recommendations.parquet` и `test_top_popular.parquet`.

1. Необходмо перейти в папку сервиса:
```
cd mle-project-sprint-4-v001/recsys
```
2. В первом терминале необходимо запустить:
```
uvicorn features_service:app --port 8010
```
3. Во втором терминале необходимо запустить:
```
uvicorn events_service:app --port 8020
```
4. В третьем терминале необходимо запустить:
```
...
Для тестиования сервиса необходимо в отдельном терминале запустить из коренвой директории:
```
